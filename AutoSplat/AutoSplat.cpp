...
int DrawSegments2Buffer(SEGMENT* pSegments)
{
	...
	for (int i = 0; i < 256; i++)
	{
		for (int j = 0; j < 16; j++)
		{
			CopyTIM2Buffer(_TIMXPOS(pSegments[i].strTilePolyStruct[j].cTileRef), _TIMYPOS(pSegments[i].strTilePolyStruct[j].cTileRef), _MAPXPOS(mapIndex), _MAPYPOS(mapIndex), tileRot);
			j++;
		}
		i++;
	}
